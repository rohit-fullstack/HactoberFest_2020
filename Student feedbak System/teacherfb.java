...
import java.awt.*;
import java.io.*;
class teacherfb extends JFrame implements ActionListener
{
    static teacherfb f;
    Container c;
    ...
    teacherfb(String s)
    {
        ...
        div=new JLabel("Div :-");
        inst=new JLabel("* Indicate peformance of student by selecting perticular option");
        q[0]=new JLabel("A . Communication skill :-");
        q[1]=new JLabel("B . average marks obtained :-");
        q[2]=new JLabel("C . total presenty :-");
        q[3]=new JLabel("D . Behavier in campus :-");
        q[4]=new JLabel("E . Overall intrest created in study :-");
        q[5]=new JLabel("F . Assignment submition :-");
        q[6]=new JLabel("G . follow discipline :-");
    ...
    public void actionPerformed(ActionEvent e)
    {
           if(e.getSource()==send)
        {
            ...
            {
                try { 
                    FileOutputStream fos=new FileOutputStream("_"+s1+".txt");
                    DataOutputStream dos=new DataOutputStream(fos);
                    dos.writeBytes(s1+" "+deptcb.getSelectedItem()+" "+yearcb.getSelectedItem()+" "+divcb.getSelectedItem()
